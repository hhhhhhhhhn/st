...
plumb(char *sel) {
	...
	switch(child = fork()) {
		...
		default: // Parent
			...
			int length;
			while ((length = read(pipefd[0], command_output, sizeof(command_output)-1)) > 0) {
				command_output[length] = '\0';
				ttysend(&((Arg){.s = command_output}));
			}
...
cresize(int width, int height)
{
	...
	win.hborderpx = (win.w - col * win.cw) * anysize_halign / 100 + borderpx;
	win.vborderpx = (win.h - row * win.ch) * anysize_valign / 100 + borderpx;
...
xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len, int x, int y)
{
	...
	/* Render underline before the glyphs */
